...
- The `session` slice (`src/store/session/sessionSlice.ts`) holds the logged-in client (`CurrentUser | null`, `= UserDTO` — see Backend integration) plus a `hydrated` flag, via `login`/`logout`/`hydrate` actions. `useAuth()` wraps login/logout; `useRequireAuth()` (used by the `(withBottomChrome)` layout and `CrearActividadPage`) redirects to `/login` once `hydrated` is true and there's still no user. There's no real backend auth yet: `/login` (`src/components/pages/LoginPage.tsx`) just lets you pick from a mock roster (`src/data/mockUsers.ts`) and calls `login()` directly — no JWT. Wire the real auth flow into the same `login`/`logout` actions when it lands.
- The session **persists across reloads** via `localStorage` (`src/store/session/sessionPersistence.ts`): a middleware mirrors every `login`/`logout` into storage, and `StoreProvider` restores it in a `useEffect` on mount, then dispatches `hydrate`. That restore is deliberately *not* done synchronously in `makeStore()`'s initial state — `localStorage` doesn't exist during SSR, so the store must start identically on server and client (logged out, `hydrated: false`) and only flip to the persisted user after mount, or React flags a hydration mismatch. Don't read `localStorage` from a component body or a lazy `useState` initializer for the same reason; only from an effect.
