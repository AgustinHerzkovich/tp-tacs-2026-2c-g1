...
public class StatisticsService {
  ...
  public WeatherForecastStatisticsResponse getWeatherForecastServiceStatistics(LocalDate startDate, LocalDate endDate) {
    try {
      ...
      throw new CouldNotRetrieveStatisticsException("Could not retrieve weather forecast service statistics", e);
    }
  ...
  private ActivityStatisticsResponse toResponse(ActivityStatistics activityStatistics) {
    return new ActivityStatisticsResponse(
      activityStatistics.getActivityStatus(),
      activityStatistics.getTotalActivities()
    );
  }

  public ActivityStatisticsResponse getActivityStatistics(ActivityStatus activityStatus) {
    try {
      ActivityStatistics activityStatistics =  statisticsAdapter.getActivityStatistics(activityStatus);
